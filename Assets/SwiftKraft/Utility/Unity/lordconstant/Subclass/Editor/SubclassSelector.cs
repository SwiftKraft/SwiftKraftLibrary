...
public class SubclassSelector
{
    ...
    public SubclassSelector(System.Type InType, System.Type InDefault = null, bool InIncludeSelf = false)
    {
        ...
        System.Type[] foundClasses = System.AppDomain.CurrentDomain.GetAssemblies()
         ...
         .Where(type => type.IsSubclassOf(InType) && !type.IsAbstract).ToArray();

        if (InIncludeSelf)
        {
            m_subClasses.Add(InType);

            if (InDefault != null && InDefault.GetType() == InType)
                m_selectedType = 0;
        }
    ...
    public object CreateSelected() => m_subClasses.InRange(m_selectedType) ? System.Activator.CreateInstance(m_subClasses[m_selectedType]) : null;

    public System.Type GetClassType() => m_subClasses[m_selectedType];
}
